...
public class Main {
    ...
    static int[] parent;    
    static long[] dist;
    static ArrayList<Edge2>[] lista;
    static long INF = Long.MAX_VALUE - 10000;
    static int[] visible;
    ...
    public static void main(String[] args) throws IOException{
        ...
        for(int i=0; i<N; i++) {
            visible[i] = Integer.parseInt(st3.nextToken());
        }

        dist = new long[N];
        lista = new ArrayList[N+1];

        for(int i=0; i<N; i++) lista[i] = new ArrayList<>();
        ...
        for(int i=0; i<M; i++) {
            ...
            lista[a].add(new Edge2(b, c));
            lista[b].add(new Edge2(a, c));  
    ...
    static void dijkstra(int start) {
        ...
        while(!pq.isEmpty()){
			...
			for(Edge2 e : lista[info.idx]){
				if(dist[info.idx] + e.weight >= dist[e.to])	continue;
                if(e.to != N - 1 && visible[e.to] == 1) continue;				
